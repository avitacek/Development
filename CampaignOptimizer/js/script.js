...
$(function() {
	...
	$.ajax('ajax.php?a=templates').done(function(data) {
		//To Do://
		//I want Master to always be defualt template choice, later can be dependent on user log in//
		// I would like to remove the _)1...ect in the html dext, not in the data module//
		allTemplates = data;

		for (var tkey in data) {
			var template = data[tkey];
			//var defaultTemp = data[tKey+1];
				...
				for (var gkey in template.modules) {
					if (template.modules[gkey].modules.length > 0) {
						...
						for (var mkey in template.modules[gkey].modules) {
							...
							$('.module-list[data-group="' + gkey + '"]').append('<li><a href="#" class="module ' +module.name+ '" data-group="' + gkey + '" data-key="' + mkey + '" data-module="' + module.name + '">'+ module.name + '</a></li>');
							//<span class="thumb"><img src="templates/' +template.name+'/modules/ModuleImages/'+template.modules[gkey].modules[mkey].name+'.jpg"/></span>//
						}
					}
	...
	$('body').on('click', '.module', function(e) {
		...
		$('#selected-modules').append('<li><a href="#" data-group="' + $(this).data('group') + '" data-key="' + $(this).data('key') + '" data-module="' + $(this).data('module') + '">' + $(this).text() + ' <span class="left-controls"><span class="move-down"></span><span class="move-up"></span></span><span class="controls"><span class="remove"><img src="../img/trash.png" width="15" height"19"/></span></span></a></li>');
		createPreview();
	});
		...
		if ($('#preview-trigger').is(':checked')) {
			if (e.type == 'mouseenter') {
				...
				setTimeout(function() {
					...
					$('#module-preview').css('top', ($('.main').innerHeight() / 4) - ($('#module-preview').contents().find('body').height() / 4));
	...
	$( "#selected-modules" ).disableSelection();
